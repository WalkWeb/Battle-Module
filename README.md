...
- Один раз при запуске боя через `public/index.php` получил ошибку "Can't be used action", при этом при последующих 
вызовах она не повторялось. Нужно сделать тест, который будет прогонять сотню боев с параметрами, как в `public/index.php`
...
- Добавить проверку+тест на возможность использования способности лечения (сейчас там стоит заглушка `return true`)
